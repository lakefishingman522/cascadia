...
func (k *Keeper) WithChainID(ctx sdk.Context) {
	chainID, err := cascadiatypes.ParseChainID(ctx.ChainID())
	if err != nil {
		panic(err)
	}
	...
	if chainID.Cmp(big.NewInt(6102)) != 0 {
		panic("EVM only supports Cascadia chain identifiers (6102)")
	}
...
func (k *Keeper) GetAccountWithoutBalance(ctx sdk.Context, addr common.Address) *statedb.Account {
	...
	ethAcct, ok := acct.(cascadiatypes.EthAccountI)
	if ok {
