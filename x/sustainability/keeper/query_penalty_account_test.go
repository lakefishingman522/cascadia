package keeper_test

import (
	"testing"

	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/stretchr/testify/require"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"

	keepertest "github.com/cascadiafoundation/cascadia/testutil/keeper"
	"github.com/cascadiafoundation/cascadia/testutil/nullify"

	"github.com/cascadiafoundation/cascadia/x/sustainability/types"
)

func TestPenaltyAccountQuery(t *testing.T) {
	keeper, ctx := keepertest.SustainabilityKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	item := createTestPenaltyAccount(keeper, ctx)
	for _, tc := range []struct {
		desc     string
		request  *types.QueryGetPenaltyAccountRequest
		response *types.QueryGetPenaltyAccountResponse
		err      error
	}{
		{
			desc:     "First",
			request:  &types.QueryGetPenaltyAccountRequest{},
			response: &types.QueryGetPenaltyAccountResponse{PenaltyAccount: item},
		},
		{
			desc: "InvalidRequest",
			err:  status.Error(codes.InvalidArgument, "invalid request"),
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.PenaltyAccount(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
				require.Equal(t,
					nullify.Fill(tc.response),
					nullify.Fill(response),
				)
			}
		})
	}
}
