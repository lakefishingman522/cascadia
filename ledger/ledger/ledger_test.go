...
func (suite *LedgerTestSuite) TestCascadiaLedgerDerivation() {
	testCases := []struct {
		name     string
		mockFunc func()
		expPass  bool
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			derivationFunc := ledger.CascadiaLedgerDerivation()
...
func (suite *LedgerTestSuite) TestGetAddressPubKeySECP256K1() {
	...
	addr := crypto.PubkeyToAddress(privKey.PublicKey)
	expAddr, err := sdk.Bech32ifyAddressBytes("cascadia", common.HexToAddress(addr.String()).Bytes())
	suite.Require().NoError(err)
