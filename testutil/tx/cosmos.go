...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdk.NewIntFromUint64(uint64(feeAmt))) // 0.01 CC
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'cascadia_11029-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appCascadia.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appCascadia.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
