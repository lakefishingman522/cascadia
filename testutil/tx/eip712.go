...
func PrepareEIP712CosmosTx(
	...
	accNumber := appCascadia.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appCascadia.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		cascadiaCodec := codec.NewProtoCodec(registry)
