...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(cascadiatypes.AttoCascadia, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(cascadiatypes.AttoCascadia, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(cascadiatypes.AttoCascadia)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, cascadiatypes.AttoCascadia, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
